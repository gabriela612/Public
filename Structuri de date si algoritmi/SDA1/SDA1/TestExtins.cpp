...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar anumite lucruri generale, nu stim in ce ordine vor fi afisate elementele.
	...
	vector<TElem> elementes;
	IteratorColectie ic5 = c4.iterator();
	while (ic5.valid()) {
		TElem e = ic5.element();
		elementes.push_back(e);
		ic5.urmator();
	}

	assert(elementes.size() == c4.dim());
	for (unsigned int i = 0; i < elementes.size(); i++) { //scoatem pe rand elemente din vectorul dat de iterator, verificam sa fie in colectie si le stergem si din vector si din c4. Incepem stergerea cu ultimul
		TElem lastElem = elementes.at(elementes.size() - i - 1);
