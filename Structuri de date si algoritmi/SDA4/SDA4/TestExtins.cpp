...
void test()
{
	...
	assert(c.elementeCuFrecventaData(1) == 10);
	for (int i = 6; i < 10; i++) {
		c.adauga(i);
	}
	assert(c.elementeCuFrecventaData(1) == 6);
	assert(c.elementeCuFrecventaData(2) == 4);
	assert(c.elementeCuFrecventaData(3) == 0);
	try
	{
		c.elementeCuFrecventaData(-2);
		throw "nu merge";
	}
	catch (exception)
	{
