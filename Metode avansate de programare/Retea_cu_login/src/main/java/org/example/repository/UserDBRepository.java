...
public class UserDBRepository implements PagingRepository<Long, Utilizator> {
    ...
    public Optional<Utilizator> findOne(Long longID) {
        try(Connection connection = DriverManager.getConnection(url, username, password);
            ...
            if(resultSet.next()) {
                ...
                Utilizator u = new Utilizator(firstName,lastName, parola);
                u.setId(longID);
    ...
    public Iterable<Utilizator> findAll() {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
            ...
            while (resultSet.next())
            {
                ...
                String lastName=resultSet.getString("last_name");
                String parola = resultSet.getString("parola");
                Utilizator user = new Utilizator(firstName,lastName, parola);
                user.setId(id);
                users.add(user);
    ...
    public Optional<Utilizator> save(Utilizator entity) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password)) {
            ...
            String insert = "INSERT INTO users(first_name, last_name, parola) VALUES (?, ?, ?)";
            PreparedStatement statement = conn.prepareStatement(insert, Statement.RETURN_GENERATED_KEYS);
            statement.setString(1, entity.getFirstName());
            statement.setString(2, entity.getLastName());
            try {
                statement.setString(3, Criptare.Cri(entity.getParola()));
    ...
    public Optional<Utilizator> update(Utilizator entity) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password)) {
            String sql = "UPDATE users SET first_name=?, last_name=?, parola=? WHERE id=?";
            PreparedStatement statement = conn.prepareStatement(sql);
            statement.setString(1, entity.getFirstName());
            statement.setString(2, entity.getLastName());
            try {
    ...
    public Page<Utilizator> findAll(Pageable pageable) {
        ...
        try (Connection conn = DriverManager.getConnection(url, username, password)) {
            ...
            while (resultSet.next()){
                ...
                String lastName = resultSet.getString("last_name");
                String parola = resultSet.getString("parola");
                Utilizator currentUser = new Utilizator(firstName, lastName, parola);
