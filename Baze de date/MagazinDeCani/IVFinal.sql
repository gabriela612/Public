...
CREATE TABLE CMateriale_folosite (
	...
	id_cana INT FOREIGN KEY REFERENCES CCani(id_cana) ON UPDATE CASCADE,
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO CCani(model, capacitate, pret, id_furnizor)
		VALUES('Cana simpla' + CAST(@i as VARCHAR), 250, 20, 1);
...
AS
BEGIN
	DECLARE @inc datetime, @fin datetime
	DECLARE @t1ins datetime, @t2ins datetime
	DECLARE @t1del datetime, @t2del datetime
	DECLARE @v1sel datetime, @v2sel datetime
	DECLARE @id_test_run INT
	DECLARE @Table varchar(255), @NrRows INT, @id_tabel INT
	DECLARE @View VARCHAR(255), @id_view INT

	INSERT INTO TestRuns(Description) VALUES ('All tests')
	SELECT @id_test_run = MAX(T.TestRunID) FROM TestRuns AS T
	...
	SELECT ta.Name, ta.TableID, tt.NoOfRows
	FROM dbo.TestTables AS tt
	INNER JOIN dbo.Tables AS ta
	ON ta.TableID = tt.TableID
	WHERE TestID = 4
	...
	OPEN cursor_tabele;
	FETCH NEXT FROM cursor_tabele INTO @Table, @id_tabel, @NrRows;
	WHILE @@FETCH_STATUS=0 
	BEGIN

		SET @t1ins = GETDATE()
		EXEC('Ins' + @Table + ' ' + @NrRows);
		SET @t2ins = GETDATE()

		INSERT INTO TestRunTables(TestRunID, TableID, StartAt, EndAt) VALUES
		(@id_test_run, @id_tabel, @t1ins, @t2ins);
	...
	BEGIN
		...
		INSERT INTO TestRunViews(TestRunID, ViewID, StartAt, EndAt) VALUES
		(@id_test_run, @id_view, @v1sel, @v2sel);
